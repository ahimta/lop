...
pub(super) trait TournamentProvider {
  ...
  // NOTE: `download_tournament()` and `process_tournament()` only supported for
  // easier testing.
  fn download_tournament() -> Vec<String>;
  fn process_tournament(downloaded_tournament: Vec<String>)
    -> Vec<MatchResult>;

  fn fetch_tournament() -> Tournament {
    let matches_results = Self::process_tournament(Self::download_tournament());
    Self::postprocess_tournament(matches_results)
  }

  #[allow(clippy::too_many_lines)]
  fn postprocess_tournament(matches_results: Vec<MatchResult>) -> Tournament {
  ...
  fn test_fetch_tournament() -> Tournament {
    let matches_results =
      Self::process_tournament(Self::test_helper_download_tournament());
    Self::postprocess_tournament(matches_results)
  }

  fn test_helper_generate_downloaded_tournament() {
    use std::fs::File;
    use std::io::Write;

    use chrono::prelude::Utc;

    let responses = Self::download_tournament();
  ...
  fn test_helper_download_tournament() -> Vec<String> {
    use std::fs;
